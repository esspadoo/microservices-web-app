            ...
            if file.endswith(".parquet"):
                ...
                try:
                    ...
                    if 'curlielabels_en' in df.columns:
                        # Mantieni solo righe con la categoria
                        ...
                        if not filtered_df.empty:
                            # Calcola percorso di destinazione mantenendo la struttura
...
def copy_and_filter_language_eng(base_dir, dest_dir, category="Computers"):
    """
    Per ogni sottocartella immediata di base_dir, trova la prima cartella
    'language=eng' e filtra tutti i .parquet per la categoria.
    """
            ...
            for root, dirs, _ in os.walk(src_path):
                for dir_name in dirs:
                    if dir_name.lower() == "language=eng":
                        ...
                        break  # Stop dopo il primo language=eng
                else:
                    continue
                break

def main():
    base_directory = "./DATASETX/"
    destination_directory = "./Prove/"
    category = "Computers"  # Macro-categoria desiderata
